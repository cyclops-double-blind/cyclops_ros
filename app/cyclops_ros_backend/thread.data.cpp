...
namespace cyclops {
  ...
  static auto parse_vector3_message(geometry_msgs::Vector3 const& v) {
  ...
  void DataThreadSpinner::handleImu(ImuConstPtr const& msg) {
    ...
    auto a = parse_vector3_message(msg->linear_acceleration);
    auto w = parse_vector3_message(msg->angular_velocity);
  ...
  void DataThreadSpinner::handleFeatures(
    ...
    auto data = image_data_t {msg->header.stamp.toSec(), {}};
    for (auto const& feature : msg->features) {
      ...
      data.features.emplace(feature.id, feature_point_t {u, H});
    }
